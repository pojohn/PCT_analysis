...
- 由統計值與圖表來看，這三項檢驗值(Response)它們的分布情況有些許差異，但 P/N 本身的範圍(range)可以說是重疊的，這表示基本上要漂亮的分出 P/N的差別是困難的
 

##第三部份：決策樹(Decision Tree)
...
```{r  Ball  P/N  tree, echo=FALSE}
############
fit <- rpart(formula = `Ball-Version` ~ `Ball-Shear`+`Ball-Size(X)`+`Ball-Size(Y)`+`Ball-Size(Z)`, data = BV, method = 'class')
...
- 在 K-W test 不同因子(Factor)會使 P/N 板子在檢驗值(Response) 有統計意義上的顯著差異，但也有可能是製程波動造成的，建議是增加樣本數後再進行檢定
- 製程本身的波動可能是以片或區為單位，只要樣本數足夠的前提下應該不會影響到檢驗結果
- 在 M-U test 結果中同樣因子(Factor) N Ball在X、Y高於P Ball，在Z低於P Ball，但 P/N 檢驗值(Response) 的範圍(range) 可以說是重疊的，這表示基本上要漂亮的分出 P/N的差別是困難的
- 由決策樹(Decision Tree)的圖可以發現當 P/N 分別明顯時該組所占比例都是偏低的，這代表 P/N  本身的範圍(range) 可以說是重疊的，這也代表一般的分類法都無法漂亮的分出 P/N 的差別