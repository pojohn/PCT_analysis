...
- 第二部份的分析檢查出在同樣因子(Factor)下的檢驗值(Response)在Ball-Version P的Ball-Size(Y)兩組的中位數並不相同 ，其K-W test 的p值<0.05
- 這代表製程本身的波動在小樣本時不容忽視，這也表示接下來在同因子的 P/N Ball 的差異也有可能只是製程波動而不是真的有差異
